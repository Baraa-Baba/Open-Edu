...
export default function Form() {
  ...
  let secsOptions = [{name: 'GS'},{name: 'LS'},{name: 'ES'},{name: 'LH'}, {name:'SAT'}]
  ...
  const [isShowReqried2,setisShowReqried2]=useState(false)
...
console.log(ApplicationTypes)
  ...
  const allSections = [{name:'GS',subjects:GSSubjectOptions},{name:'ES',subjects:ESSubjectOptions},{name:'LS',subjects:LSSubjectOptions},{name:'LH',subjects:LHSubjectOptions}, {name:'SAT',subjects:SATSubjectOptions}]
...
const uploadFile = (file,fileID,singleFileData,fileName) => { 
  ...
  const uploadTask = uploadBytesResumable(sotrageRef, file); 
   uploadTask.on(
    ...
    useEffect(()=>{
      console.log(document.querySelector('.singleSelect #subjectMutli_input'))
      document.querySelector('.singleSelect #subjectMutli_input').disabled=false
      document.querySelector('.singleSelect #subjectMutli_input').addEventListener('click',(e)=>{
        e.stopPropagation() 
      })
    },[])
    return (
...
{ !selectedSubject ||  AvialibleUntis[0]?.name  ? <>
   ...
   {isClickedOnSubmit &&
    ...
    <Multiselect showArrow onRemove={(value) => handleApplicationTypes(value)}  onSelect={(value) => handleApplicationTypes(value)}
     className='mutilSelect' avoidHighlightFirstOption options={[...typeApplcation,{name:'books'},{name:'exams'}]}  placeholder='Types' displayValue="name" showCheckbox={true} />
    ...
    <div className="">
        <div className="form-floating  mb-4">
            <input accept="application/pdf" type="file" onChange={(e) => handleFileInputChange(e.target)} id="form_file" multiple="multiple" name="file[]" className="form-control  upload-empty" placeholder="Attach File" required="" data-gtm-form-interact-field-id="0" />
