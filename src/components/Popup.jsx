import React, { useEffect } from 'react'
import '../css/Popup.css'
export default function Popup({isFileUploaded,fileLength}) { 
  return (
    <div className='PopupCont'> 
        {isFileUploaded>= fileLength ?
         <div>
            <div class = "confirm">
        <div class="loading">
            <div class="circle" id = "main">
                <div class="circle" id = "moving">
                    <div id = "second"> 
                        <p>
                            <svg xmlns="http://www.w3.org/2000/svg" width="150" height="150" fill="#27c857" class="bi bi-check-lg" viewBox="0 0 16 16">
                            <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425z"/>
                          </svg>
                        </p>
                    </div>
                </div>
            </div>
        </div>
        <div class = "thanks">
            <h2> Thank You!</h2>
            <p>Please wait up to 48hrs for your contribution to appear.</p>
        </div>
        <div class = "othershit">
          <button><a href="/form">Contribute</a></button>
          <button><a href = "/">Learn</a></button>
        </div>
    </div>
        </div>: 
        <div className='loadingCont'>
 <div class="lds-ellipsis">
    <div></div>
    <div></div>
    <div></div>
    <div></div>
              </div>
              <div class = "thanks">
            <h2> uploading documents please wait</h2> 
        </div>
        </div>
        }
    </div>
  )
}
